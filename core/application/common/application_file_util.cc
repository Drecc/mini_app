...
//   }
// }
std::unique_ptr<base::DictionaryValue> LoadXMLNode(
  ...
  for (xmlNode* node = root->children; node; node = node->next) {
    ...
    auto sub_value = LoadXMLNode(node, current_dir);
    if (!sub_value)
      continue;
...
std::unique_ptr<Manifest> LoadManifest<Manifest::TYPE_WIDGET>(
  ...
  doc = xmlReadFile(manifest_path.MaybeAsASCII().c_str(), nullptr, 0);
  if (doc == nullptr) {
    *error = base::StringPrintf("%s", errors::kManifestUnreadable);
    return std::unique_ptr<Manifest>();
  }
  root_node = xmlDocGetRootElement(doc);
  auto dv = LoadXMLNode(root_node);
